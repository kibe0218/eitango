...
class SignUpViewModel:  NSObject, ObservableObject {
    ...
    // 最終判定
    func signUp(method: AuthMethod) async {
        do {
            session.user = try await useCase.divideMethodAndSignUp(method: method)
        } catch {
...
extension SignUpViewModel: ASAuthorizationControllerDelegate {
    ...
    func authorizationController(
        ...
        if let appleIDCredential = authorization.credential as? ASAuthorizationAppleIDCredential {
            ...
            Task {
                await signUp(method: .apple(idToken: idTokenString, nonce: nonce))
            }
        }
    }
}
